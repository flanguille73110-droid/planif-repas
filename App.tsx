    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-fadeIn pb-10">
      ...
      <div className="bg-white p-6 rounded-[32px] border border-purple-100 shadow-sm space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
          ...
          <div className="sm:col-span-2"><input type="number" className="w-full p-3.5 border border-gray-100 rounded-2xl bg-gray-50 font-black text-center text-purple-600 outline-none" value={newItemAmount} onChange={e => setNewItemAmount(Number(e.target.value))} /></div>
        ...
        {items.length === 0 ? <div className="p-20 text-center text-gray-300 italic font-medium">Réserve vide.</div> : items.sort((a,b)=>a.name.localeCompare(b.name)).map(i => (
          <div key={i.id} className="p-5 flex gap-5 items-center group">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <input type="number" className="w-20 p-2 text-center font-black text-sm bg-purple-50 text-purple-600 rounded-xl outline-none" value={i.amount} onChange={(e) => setItems(items.map(x => x.id === i.id ? { ...x, amount: Number(e.target.value) } : x))} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-12 gap-3 bg-white p-4 border rounded-[28px]">
            <input type="number" className="col-span-3 p-2 border rounded-xl" value={pending.amount} onChange={e => setPending({ ...pending, amount: Number(e.target.value) })} />
...
const RecurringView: React.FC<{ groups: PantryGroup[]; setGroups: React.Dispatch<React.SetStateAction<PantryGroup[]>>; foodPortions: FoodPortion[]; onAddFoodToSettings: (name: string, unit: string) => void; onSendToShopping: (items: ShoppingListItem[]) => void; }> = ({ groups, setGroups, foodPortions, onAddFoodToSettings, onSendToShopping }) => {
  ...
  // Function to load a group into the editor
  ...
  // Function to update the amount of an item within a specific group
        ...
        <div className="bg-white p-8 rounded-[40px] border shadow-2xl space-y-8 animate-slideDown">
          ...
          <div className="bg-purple-50 p-6 rounded-3xl space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
              ...
              <div className="sm:col-span-2"><input type="number" className="w-full p-4 rounded-2xl border font-black text-center text-purple-600 outline-none" value={niAmt} onChange={e => setNiAmt(Number(e.target.value))} /></div>
